...
function StaffRow({ row, score }: StaffRowProps) {
  return (
    <g>
      {/* Staff lines */}
      ...
      {row.measures.map((measure, mIdx) => (
        <g key={mIdx} transform={`translate(${measure.offsetX + CLEF_WIDTH}, 0)`}>
          {measure.notes.map((slot) => (
            <StaffNote key={`note-${slot.x}`} slot={slot} score={score} />
          ))}
...
function TrebleClef({ x, y }: { x: number; y: number }) {
  // Bravura SMuFL U+E050 treble clef, pre-scaled and centered so the glyph
  // spans roughly four staff spaces and sits on the middle line.
...
function StaffNote({ slot, score }: { slot: NoteSlot; score: ScoreAST }) {
  const { item, x } = slot;
  ...
  return (
    <g>
      ...
      <NoteHead x={x} y={y} durationType={durationType} />

      {durationType !== "whole" && <Stem x={stemX} y1={y} y2={stemEndY} />}

      {(durationType === "eighth" || durationType === "sixteenth") && (
        <Flag x={stemX} y={stemEndY} stemUp={stemUp} double={durationType === "sixteenth"} />
      )}

      {note.duration.dotted && (
        <circle cx={x + 10 * STAFF_SCALE} cy={y} r={2 * STAFF_SCALE} className="fill-on-surface" />
      )}
...
function NoteHead({ x, y, durationType }: { x: number; y: number; durationType: string }) {
  const common = {
    cx: x,
    cy: y,
    rx: NOTEHEAD_RX,
  ...
  return <ellipse {...common} className="fill-on-surface" strokeWidth={0} />;
}

function Stem({ x, y1, y2 }: { x: number; y1: number; y2: number }) {
  return <line x1={x} y1={y1} x2={x} y2={y2} stroke="#1F2937" strokeWidth={STROKE_WIDTH} strokeLinecap="round" />;
}

function Flag({ x, y, stemUp, double }: { x: number; y: number; stemUp: boolean; double: boolean }) {
  ...
  return (
    <g>
      <path d={d} fill="none" stroke="#1F2937" strokeWidth={STROKE_WIDTH} strokeLinecap="round" />
      {double && <path d={d2} fill="none" stroke="#1F2937" strokeWidth={STROKE_WIDTH} strokeLinecap="round" />}
    </g>
  );
}
