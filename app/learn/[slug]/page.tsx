...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  if (!lesson) return {};
  return {
    title: `${lesson.title} | NotaBridge Learn`,
    description: lesson.description,
  };
}

export default async function LessonPageEn({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const lesson = getLessonBySlug(slug);
  if (!lesson) notFound();
    ...
    <>
      ...
      <main className="flex-grow max-w-[1280px] mx-auto w-full px-4 md:px-16 py-8">
        <div className="flex flex-col gap-4 mb-8">
          <Link href="/learn" className="text-sm font-medium text-secondary hover:text-primary w-fit inline-flex items-center gap-1">
            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M10 19l-7-7m0 0l7-7m-7 7h18" />
            </svg>
            Back to Learn
          </Link>
          <h1 className="text-3xl font-semibold text-on-surface">{lesson.title}</h1>
          <p className="text-lg text-on-surface-variant">{lesson.description}</p>
        </div>
